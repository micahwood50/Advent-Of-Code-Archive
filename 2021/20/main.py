...
def get_nine_input_grid(
    input_image: set[tuple[int, int]], set_ch: str, x: int, y: int
) -> list[list[str]]:
    grid = list()
...
def get_output_image(
    input_image: set[tuple[int, int]], set_ch: str, image_enhancement: str
) -> set[tuple[int, int]]:
    visited_set = set()
    output_set = set()
    ...
    for px, py in input_image:
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                pixel = get_output_pixel(
                    get_nine_input_grid(input_image, set_ch, *coord), image_enhancement
                )
...
def part_1():
    ...
    result = len(
        get_output_image(
            get_output_image(light_pixel_set, LIGHT_PIXEL, image_enhancement),
            DARK_PIXEL,
            image_enhancement,
        )
    )
