...
def password_generator(password: str) -> str:
    ...
    def check_requirement(password_array: list[int]) -> bool:
        ...
        for i in range(len(password_array)-2):
            if password_array[i] + 1 == password_array[i+1] == password_array[i+2] - 1:
                break
        ...
        # 2nd Requirement
        if ord('i')-ord('a') in password_array or ord('l')-ord('a') in password_array or ord('o')-ord('a') in password_array:
            return False
