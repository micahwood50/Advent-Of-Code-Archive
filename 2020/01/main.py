...
def part_1():
    ...
    for entry in entries_set:
        if GOAL_NUM - entry in entries_set:
            print(
                f"The product of the two entries that sum to 2020 is {entry * (GOAL_NUM - entry)}"
            )
            return
    ...
    for entry_1 in entries_set:
        for entry_2 in entries_set:
            ...
            if GOAL_NUM - entry_1 - entry_2 in entries_set:
                print(
                    f"The product of the three entries that sum to 2020 is {entry_1 * entry_2 * (GOAL_NUM - entry_1 - entry_2)}"
                )
                return
