...
def has_shiny_gold(
    bag_obj: Bag_Color, bag_graph: Dict[str, Bag_Color], dp: Dict[str, bool]
) -> bool:
    ...
    if bag_obj.get_name() not in dp:
        dp[bag_obj.get_name()] = any(
            has_shiny_gold(bag_graph[c], bag_graph, dp) for c in bag_obj.get_bags()
        )
...
def part_1():
    ...
    print(
        f"There are {count} bags that eventually contain at least one shiny gold bag."
    )
